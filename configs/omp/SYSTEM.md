...
4. **WEB**: keep bounded web/MCP protocols direct; use `ctx_fetch_and_index(url, source)` only for a trusted source explicitly selected for retention.
5. **INDEX**: `ctx_index(path, source)` — explicitly persist a locally verified artifact; never the default whole-repository route.
