...
import { sourcePreviewChars } from "./response-measurement-utils.mjs";

const STDERR_TAIL_MAX = 2_048;

function discoverRepoRoot(anchor) {
...
function resolveEntry(argv, repoRoot) {
  const entryIndex = argv.indexOf("--entry");
  if (entryIndex === -1) return join(repoRoot, "start.mjs");
  const value = argv[entryIndex + 1];
  if (!value || value.startsWith("--")) {
    throw new Error("Usage: node scripts/measure-response-sizes.mjs [--entry <path>]");
  }
  return isAbsolute(value) ? value : resolve(process.cwd(), value);
}

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = discoverRepoRoot(scriptDir);
const entry = resolveEntry(process.argv.slice(2), repoRoot);
...
function metrics(text) {
  return {
    ...
    lines: text.split("\n").length,
    sourcePreviewChars: sourcePreviewChars(text),
  };
}

function sanitizeStderr(value, fixture) {
  return value
    .replaceAll(fixture, "<fixture>")
    .replace(/(authorization|token|api[_-]?key|secret)(\s*[:=]\s*)\S+/gi, "$1$2<redacted>")
...
for (const scenario of scenarios) {
  ...
  transport.stderr?.on("data", (chunk) => {
    stderrTail = sanitizeStderr(`${stderrTail}${String(chunk)}`, fixture);
  });
  ...
  try {
    ...
    throw new Error(
      `response measurement ${scenario.name} failed: ${message}; stderrTail=${JSON.stringify(stderrTail)}`,
      { cause: error },
    );
  } finally {
