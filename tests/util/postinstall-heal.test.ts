...
// ─────────────────────────────────────────────────────────────────────────
// Slice 9 — runtime-version-neutral installation
// ─────────────────────────────────────────────────────────────────────────

describe("postinstall — runtime-version-neutral installation", () => {
  it("does not declare a package-level Node engine gate", () => {
    const pkg = JSON.parse(readFileSync(resolve(REPO_ROOT, "package.json"), "utf-8"));
    expect(pkg.engines).toBeUndefined();
  });

  it("does not hard-fail installation based on the host Node version", () => {
    const src = readFileSync(REPO_POSTINSTALL, "utf-8");
    expect(src).not.toMatch(/process\.platform\s*===\s*["']linux["']/);
    expect(src).not.toMatch(/process\.versions\.node/);
